invoke_git_browse_open() {
  ...
  run_detached_command() {
    ...
    if command_exists setsid; then
      if setsid -f zsh -lc "$command_string" >/dev/null 2>&1 </dev/null; then
        return 0
      fi
    fi

    nohup zsh -lc "$command_string" >/dev/null 2>&1 </dev/null &
...
invoke_git_browse() {
  ...
  printf 'Example: export GIT_BROWSE_OPEN_CMD='\''open {url}'\''\n'
}
